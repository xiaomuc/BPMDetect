    ...
    class BpmDetector
    {
        ...
        public int detect(String fileName)
        {
            ...
            foreach (int bpm in _peaks)
            {
                ...
                else if (p / peakMax > _threshold && _priorityBpmLow <= bpm && bpm < _priorityBpmHigh && (bMax % bpm == 0 || bpm % bMax == 0))
                {
                    bMax = bpm;
                }
            }
        ...
        public int getVolume()
        {
            ...
            int read;
            float[] buff = new float[_windowSize];
            while ((read = _sampleSource.Read(buff, 0, _windowSize)) == _windowSize)
            {
                double v = 0;
        ...
        public void calcBpm()
        {
            double s = (double)_sampleSource.WaveFormat.SampleRate / (double)_windowSize;
